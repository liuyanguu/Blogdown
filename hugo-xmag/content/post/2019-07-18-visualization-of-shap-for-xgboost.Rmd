...
I wrote the R package [**SHAPforxgboost**](https://cran.r-project.org/web/packages/SHAPforxgboost/index.html) to cover all the plotting functions illustrated in this post. This post serves as the vignette for the package.
...
mod <- xgboost::xgboost(data = dataX, 
                        ...
                        params = param_list, nrounds = param_list$nrounds,
...
pred_mod <- predict(mod, dataX, ntreelimit = param_list$nrounds)
...
So in short, the order/structure of how the tree is built doesn't matter for SHAP, but matters for Gain, and the mean absolute SHAP is the same (20 vs. 20). The SHAP scores (SHAP.Fever, SHAP.Cough) for model `m1` and `m2`:
Model `m1`: 
```{r, echo = FALSE}
