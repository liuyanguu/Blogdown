...
* It is unnecessary to add **Depends** in the **DESCRIPTION** except `R (>= 3.3.0)`. And the **Imports** part in **DESCRIPTION** is irrelevant to what you import into NAMESPACE in the code (although they are both named _import_).  The NAMESPACE is defined by using `@import` to import packages and functions in the R code. For now the only thing to depend on is "R". 

![](/post/2019-07-28-some-experience-on-writing-r-package_files/the cran page.PNG){width=60%}
