...
By combining the advantages from both random forest and gradient boosting, XGBoost gives prediction error 10-times lower than random forest or gradient boosting alone in my case. 
...
sdi <- read_excel("D:/OneDrive - nyu.edu/NYU Marron/Data/20180618_SDI 2.xlsx", na = "", skip = 1)
...
* Tuning by simulation: randomized parameters and update the record with best ones. 
* It turns out to be a very interesting and good method to scan for hyperparameters.
...
* The plots are not shown in pdf, but can be viewed _[on my blog](https://yangliuresearch.blogspot.com/2018/07/extreme-gradient-boosting-xgboost.html)_
```{r boostplot}
library("DiagrammeR")
...
# Data Cleaning -----------------------------------------------------------
sdi <- read_excel("D:/OneDrive - nyu.edu/NYU Marron/Data/20180618_SDI 2.xlsx", 
                  na = "", skip = 1)
...
sdi[sapply(sdi, is.character)] <- lapply(sdi[sapply(sdi, is.character)], 
                                       as.factor)
sdi$Share_Temporary <- sdi$CC7_Structures_Temporary / 
  (sdi$CC7_Structures_Temporary +sdi$CC6_Structures_Permanent)
# remove variables with over 10% NA
Remove <- (sapply(sdi, function(x) sum(is.na(x))) < dim(sdi)[1]*0.1)
mydata0 <- sdi[Remove]
mydata <- na.omit(mydata0) # remove all the NA
