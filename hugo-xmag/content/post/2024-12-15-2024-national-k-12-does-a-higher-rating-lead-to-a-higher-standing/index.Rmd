...
p_scatter_group <- p_scatter + facet_wrap(~ Grade_full, ncol = 3 , scales = "free_y")
...
g_points_rating <- ggplot(combined_data, aes(x = Pts , y = Rate, color = Grade)) +
...
g_points_rating <- ggExtra::ggMarginal(g_points_rating, type = "histogram", bins = 50, size = 10, color = "white", fill = "grey")
...
g_points_rating_group <- ggplot(combined_data, aes(x = Pts , y = Rate, color = Grade)) +
  ...
  ggh4x::facet_wrap2(~ Grade_full, ncol = 3, axes = "all", scales = "free_y")
```
...
Interestingly, the correlation between rating and percentile is relatively weak. While it is statistically significant due to the large sample size, the R-squared value (from a fitted linear regression) is only `r round(r2, 2)`. This suggests that the correlation isn’t as strong as one might assume. The red lines represent a loess (locally estimated scatterplot smoothing) curve, which highlights the trend but shows limitations, particularly when examined by individual grades.
...
p_scatter_group
# ggsave("temp/scatter_facet.png", p_scatter_group, width = 12, height = 12, bg = "white")
...
# ggsave("temp/g_points_rating_group.png", g_points_rating_group, width = 12, height = 12, bg = "white")
