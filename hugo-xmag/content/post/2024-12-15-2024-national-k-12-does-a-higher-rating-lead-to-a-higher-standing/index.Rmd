...
p_scatter <- ggplot(combined_data, aes(x = Rate, y = Percentile)) +
  ...
  labs(title = "Percentile vs. Rating", x = "Rating", y = "Percentile", color = "Grade" ) +
  mytheme1 + theme(legend.position = "none") + 
  ggpubr::stat_cor(method = "pearson")

# scatterplot by grade
p_scatter_group <- ggplot(combined_data, aes(x = Rate, y = Percentile)) +
  ...
  labs(title = "Percentile vs. Rating", x = "Rating", y = "Percentile", color = "Rating" ) +
  mytheme1 + 
...
if (SAVE_png) ggsave("temp/scatter.png", p_scatter_all, width = 8, height = 6, bg = "white")
...
The 2024 National K-12 Grades Championship attracted a total of `r formatC(combined_data[,.N], format = "d", big.mark = ",")` participants from `r combined_data[, uniqueN(St)]` states. More than 1,000 participants came from New York, followed by Virginia with 250 participants and New Jersey with 218 participants.
...
Interestingly, the correlation between rating and percentile is relatively weak. While it is statistically significant due to the large sample size, the R-squared value (from a fitted linear regression) is only `r round(r2, 2)`. This suggests that the correlation isn’t as strong as one might assume. The red lines represent a LOESS (locally estimated scatterplot smoothing) curve, providing a non-linear fit.
