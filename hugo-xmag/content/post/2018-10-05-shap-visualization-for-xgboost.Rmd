...
Code is also hosted on [Github](hugo-xmag/content/post/2018-10-05-shap-visualization-for-xgboost.Rmd)
...
# SHAP summary plot -------------------------------------------------------
# make summary plot (sina plot) using long-format data:
#     variable value rfvalue stdfvalue mean_value
# 1:   dayint -0.23   11012   0.00000       0.14
# 2:   dayint -0.20   11012   0.00000       0.14
plot.shap.summary <- function(data_long, x_bound_given = NULL, dilute = F){
  require('ggforce') # for `geom_sina`
  if (dilute){
  set.seed(1234)
  data_long <- data_long[sample(nrow(data_long), nrow(data_long)/100)] # dilute
  }
  ...
  x_bound <- x_bound_given  # bound of SHAP values
  }
...
shap_result <- shap.score.rank(xgb_model = xg_mod, shap_approx = F, 
                               X_train = X_train)
shapobs <- shap.stack.data(shap_contrib = shap_result$shap_score, top_n = 7, data_percent = 1, cluster_method = 'ward.D')
shap.stack.plot(shapobs = shapobs,
                zoom_in_location  = 50,
                y_parent_limit = c(-0.3,0.4),
                # y_zoomin_limit = c(-0.3, 0.5),
                zoom_in = T)
```

### Stack plot by clustering groups

Default to show 10 groups, within each group the features present a similar pattern of contribution to prediction  
...
# default to cluster into 10 groups, here we show 8. Within each cluster, the patterns of how the predictors drive prediction are closer.

shapobs <- shap.stack.data(shap_contrib = shap_result$shap_score, top_n = 7, data_percent = 1, cluster_method = 'ward.D', n_groups = 8)
shap.stack.plot.by.group(shapobs = shapobs)
```
