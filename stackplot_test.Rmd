...
shap.stack.plot.by.group <- function(shapobs, id = 'id',
  ...
  require(ggforce)  
  # optional: location to zoom in certain part of the plot
  shapobs_long <- melt.data.table(shapobs[group==zoom_in_group,], id.vars = c(id,"group"))
  require(RColorBrewer)
  p <- ggplot(shapobs_long, aes_string(x = id, y = "value" , fill = "variable")) + 
    geom_col(width =1, alpha = 0.9) +
    # geom_area() +
...
shap.stack.plot.by.group(shapobs = shapobs, zoom_in_group = 2L)

fig_list <- lapply(1:max(shapobs$group), shap.stack.plot.by.group, shapobs = shapobs, id = 'id',y_parent_limit = NULL)
fit_grid <- gridExtra::arrangeGrob(grobs = fig_list, ncol = 2)
ggsave(fit_grid, file = "D:/liuyanguu/Figures/SHAP_grid_group.png", width = 16, height = 16)
```
